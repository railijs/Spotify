<x-app-layout>
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <h1 class="text-4xl font-bold text-white bg-gradient-to-r from-gray-800 via-gray-900 to-black bg-clip-text mb-6">
            Playlist History
        </h1>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-gray-900 divide-y divide-gray-700">
                        @foreach($playlists as $playlist)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ $playlist->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ $playlist->mood }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ $playlist->description }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ $playlist->created_at->format('Y-m-d H:i') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
